...
import RentabiliteGraph from './rentabilite-graph';
import RentabiliteCashflowGraph from './rentabilite-cashflow-graph';
...
const RentabiliteTab: React.FC<RentabiliteTabProps> = ({ property, loans }) => {
  ...
  const rentabiliteData = yearlyInterests.map((row, idx) => {
    const loyerAnnuel = Number(property?.rent) * 12 || 0;
    const charges = getAllCharges(property);
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Nouveau graphique cashflow cumulé (multi-courbes) */}
      <RentabiliteCashflowGraph data={rentabiliteData.map(row => ({
        year: row.annee,
        cashflowCumule: row.cashflowCumul,
        cashflow: row.cashflow,
        resultatNet: row.resultatNet,
        impot: row.impot,
      }))} />
      <div className="overflow-x-auto">
        <table className="min-w-full text-xs border border-gray-200 bg-white">
          ...
          <tbody>
            {rentabiliteData.map((row, idx) => {
              const highlight = breakEvenYear === row.annee;
              return (
                <tr key={row.annee} className={`text-center ${highlight ? 'bg-green-50 text-green-900 font-bold border-y-2 border-green-300' : ''}`}>
                  <td className="px-2 py-1">{row.annee}</td>
                  <td className="px-2 py-1">{row.loyerAnnuel.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</td>
                  <td className="px-2 py-1">{row.charges.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</td>
                  <td className="px-2 py-1">{row.interets.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</td>
                  <td className="px-2 py-1">{row.resultatNet.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</td>
                  <td className="px-2 py-1">{row.impot.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</td>
                  <td className="px-2 py-1">{row.cashflow.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</td>
                  <td className="px-2 py-1">{row.cashflowCumul.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</td>
                </tr>
              );
            })}
          </tbody>
